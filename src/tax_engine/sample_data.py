...
from .models import EventType, LotOrigin, SavingsIncomeYear, StockEvent


def _apply_espp_discounts(events: list[StockEvent]) -> list[StockEvent]:
    """Tag the sample's ESPP purchases with their typed origin and discount data.

    Mirrors the real pipeline: BenefitHistory.xlsx supplies the FMV and the price
    actually paid for each ESPP purchase, and the loader attaches both to the
    event so the discount travels with the lot. Here that source is
    :func:`create_sample_espp_map`. Purchases on other dates stay MARKET.
    """
    espp = create_sample_espp_map()
    for event in events:
        if event.event_type is not EventType.BUY:
            continue
        discount = espp.get(event.event_date)
        if discount is None:
            continue
        fmv_usd, paid_usd = discount
        event.origin = LotOrigin.ESPP
        event.espp_fmv_usd = fmv_usd
        event.espp_price_usd = paid_usd
    return events
