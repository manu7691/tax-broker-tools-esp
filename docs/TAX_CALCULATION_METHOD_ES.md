...
* **Aviso de doble deducción.** Esa misma declaración detecta una pérdida que va a computarse dos veces: si un ejercicio presentado refleja menos pérdida bloqueada de la que el motor calcula ahora, esa diferencia se dedujo entonces y su liberación posterior volvería a computarla. El motor **avisa y no altera ninguna cifra**. La vía que da la norma es una *complementaria* del ejercicio afectado (art. 122.2 LGT), tras la cual la integración posterior es legítima; compensar el error contra un ejercicio posterior carece de respaldo normativo, y la Administración ha considerado sancionable la compensación silenciosa hacia adelante de bases negativas improcedentes. `--forfeit-declared-releases` implementa esa segunda vía para quien decida, con su asesor, no regularizar.
