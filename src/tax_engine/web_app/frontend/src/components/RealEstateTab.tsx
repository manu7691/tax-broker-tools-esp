...
import type { ManualData, RealEstateYear } from '../types/api';
...
const defaultYear = (): RealEstateYear => ({ rental_eur: 0, gains_eur: 0, notes: '' });

export function RealEstateTab({ years, manualData, onSave }: Props) {
  const [draft, setDraft] = useState<Record<string, RealEstateYear>>({});
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (!manualData) return;
    const initial: Record<string, RealEstateYear> = {};
    for (const y of years) initial[String(y)] = manualData.real_estate[String(y)] ?? defaultYear();
    setDraft(initial);
  }, [manualData, years]);

  const handleChange = (year: number, field: keyof RealEstateYear, value: string) => {
    setDraft((prev) => ({
      ...prev,
      [String(year)]: {
        ...prev[String(year)],
  ...
  const handleSave = async () => {
    if (!manualData) return;
    setSaving(true);
    await onSave({ ...manualData, real_estate: draft });
    setSaving(false);
  };
        ...
        <table className="w-full text-sm text-zinc-300">
          <thead>
            <tr className="text-xs text-zinc-500 border-b border-zinc-700">
              ...
              <th className="text-right pr-4">Rental Income (€)</th>
                ...
                <tr key={y} className="border-b border-zinc-800">
                  ...
                  <td className="pr-4">
                    <input
                      type="number"
                      step="0.01"
                      value={row.rental_eur}
                      onChange={(e) => handleChange(y, 'rental_eur', e.target.value)}
                  ...
                  <td className="pr-4">
                    <input
                      ...
                      step="0.01"
                      value={row.gains_eur}
                      onChange={(e) => handleChange(y, 'gains_eur', e.target.value)}
                      className="w-32 text-right bg-zinc-800 border border-zinc-700 rounded px-2 py-1 text-zinc-100 focus:outline-none focus:border-blue-500"
                    />
                  </td>
                  <td className="pl-4">
