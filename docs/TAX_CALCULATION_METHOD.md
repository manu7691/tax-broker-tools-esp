...
* **Double-deduction warning.** The same declaration detects a loss scheduled to be deducted twice: if a filed year reports less blocked loss than the engine now computes, the difference was taken as deductible back then, and its later release would compute it again. The engine **reports this and changes nothing**. The remedy the law provides is a *complementaria* for the affected year (Art. 122.2 LGT), after which the later integration is legitimate; netting the error against a later year instead has no statutory backing, and the administration has treated silent forward netting of improper negative bases as sanctionable. `--forfeit-declared-releases` implements that second route for taxpayers who decide, with their advisor, not to regularise.
